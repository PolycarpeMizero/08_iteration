...
healthy plant absorb more of RGB, while reflect a lot of NIR and Red_edge and vice versa.
...
nematode
```
  wavelength reflectance ranges from 0 to 1, how much sunlight gets to plant (RGB), vs sunlight reflected back by the plant (NIR, RE),
  we see plant as green because plants reflects the green more than red and blue.
...
```
high frequencies for lower values and longer tails, skewed.
...
```
ndvi ranges form 0 to 1, the higher the ndvi, the healthier the plant.
nir relates to biomass, red relates to dark green.
...
meancv
```
if you want to bring a dataframe you assigned to a certain thing, you use {{}} for that function
...
option 2: wrap the body of the function inside the 
{r}
meancv <- function(df, col) {
  # Calculate intermediary steps of mean and sd
...
nematode_meancv
```
26 being the number of columns we will get mean and cv for
...
nematode_mods_sel
```
To select the best model, we base on r2, r squared, the best model is the one with highest r2
